...
const pool = mariadb.createPool({host: mysql_host, user: mysql_user, password: mysql_pass, connectionLimit: 1024});
...
async function oregon_character_spell_languages(source, dest) {
  ...
  try {
    ...
    for await (let row of rows) {
      if(mappings.language[row.skill] !== undefined) {
        await conn.query(`INSERT IGNORE INTO ${dest}.character_spell (guid, spell, active, disabled) VALUES (${row.guid}, ${mappings.language[row.skill]}, 1, 0)`)
      }
    }
  } catch (err) {
    throw err
...
async function migrate(source, dest, table, modifiers, basetable) {
  ...
  if(modifiers.finalize) {
    for (const [name, func] of Object.entries(modifiers.finalize)) {
      console.log(`-> Add "${table}.${name}" to finalize queue`)
  ...
  try {
    conn = await pool.getConnection();
    await conn.query("TRUNCATE " + dest + ".`" + table + "`;")
...
(async function() {
  ...
  if((script === undefined) || (sourceDB === undefined) || (destinationDB === undefined)){
    ...
    // run core migrations
    for await (const [key, value] of Object.entries(migrations[script])) {
      await migrate(sourceDB, destinationDB, key, value, migrations[script])
    }

    // run finalize functions
    for await (const [name, func] of Object.entries(finalize_queue)) {
      console.log(`Finalizing: ${name}`)
      await func(sourceDB, destinationDB)
    }
